...
public class LocalService implements ILocalService {
    ...
    //Mas cercana CON PRODUCTOS
    
    public LocalModel localMasCercano(LocalModel localModel, ProductoModel productoModel, StockService stockService) {
 	   ...
 	   for (Local local: getAll()) {
 		   ...
 		   if(localModel.calcularDistancia(local.getLatitud(), local.getLongitud())<distancia) {
 			   ...
 			   //Pero me quedo con el minimo SOLAMENTE si hay productos.... SINO SIGO BUSCANDO... 
 			   if(stockService.totalPorProductoYLocal(productoModel,  localConverter.entityToModel(local))>0) {
