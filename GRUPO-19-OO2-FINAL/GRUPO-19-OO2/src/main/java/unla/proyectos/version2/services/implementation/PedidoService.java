...
public class PedidoService implements IPedidoService {
	...
	@Override
	public String calcularSueldo(List<Pedido> pedidos) {
		//Lo hago asi nomas Wolf, sino no llegamos u_U 
		
		String recibo = "";
		
		if(pedidos.size()>0) {
		recibo = "Sueldo de: " +pedidos.get(0).getVendedor().getApellido()  +",  "
				+pedidos.get(0).getVendedor().getNombre() +". Sueldo Bruto: " +pedidos.get(0).getVendedor().getSueldoBasico() +". ";
		
		float total = pedidos.get(0).getVendedor().getSueldoBasico(); 
		
		for(Pedido p: pedidos) {
			
			float cantidadXPrecioXExtra = p.getCantidad()*p.getProducto().getPrecioUnitario()*p.getVendedor().getExtra();

			total = total + cantidadXPrecioXExtra;
			
		}
		
		recibo = recibo +"Sumando extras por las ventas: $" +total;
		
		}
		
		else {recibo = "Este mes no ha realizado ventas, cobrará solo el sueldo básico.";}
